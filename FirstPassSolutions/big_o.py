...
# Given the name of animal,
# Return True if that animal is in the list, false if it is not
...
# Return the animal's index if that animal is in the list, -1 if it is not
...
def print_animal_pairs():
    counter = 0
    for animal_1 in animals:  # nested for-loop
        for animal_2 in animals:  # you are doing O of n...n times
...
# iterate through anything is O(n)
# Anytime you nest it becomes O(n^x) where x is the number of nested levels (aka count the number of "for"'s you have)
''' EX:
for i in index: <- O(n)
    return

for i in index:
    for i_2 in index: <- O(n^2)

for i in index:
    for i in index_2: <- O(n^3)
        for i in index_3:
'''
